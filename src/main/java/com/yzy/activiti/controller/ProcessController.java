package com.yzy.activiti.controller;

import com.yzy.activiti.service.ProcessService;
import com.yzy.activiti.vo.ProcessVO;
...
public class ProcessController extends BaseController {
    ...
    public ResultDTO deploy(String exportDir, String category, MultipartFile file) {
...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
    ...
    public ResultDTO convertToModel(@PathVariable("procDefId") String procDefId, RedirectAttributes redirectAttributes) throws UnsupportedEncodingException, XMLStreamException {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return ResultDTO.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
        ...
        try {
            ...
            return ResultDTO.ok("转换模型成功，模型ID=" + modelData.getId());
        } catch (Exception e) {
            e.printStackTrace();
            return ResultDTO.ok("转换模型失败");
        }
    ...
    public ResultDTO batchRemove(@RequestParam("ids[]") String[] ids) {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return ResultDTO.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
        for (String id : ids) {
