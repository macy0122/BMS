...
import com.yzy.activiti.service.SalaryService;
import com.yzy.activiti.utils.ActivitiUtils;
...
public class SalaryController extends BaseController {
    ...
    @ResponseBody
    @PostMapping("/save")
    public ResultDTO saveOrUpdate(SalaryDO salary) {
    ...
    public ResultDTO update(SalaryDO salary) {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return ResultDTO.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
    ...
    @ResponseBody
    public ResultDTO remove(@RequestParam("ids[]") String[] ids) {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return ResultDTO.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
