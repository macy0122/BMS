...
public class ModelController extends BaseController {
    ...
    public ResultDTO remove(@PathVariable("id") String id) {
    ...
    public ResultDTO deploy(@PathVariable("id") String id) throws Exception {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return ResultDTO.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
        //获取模型
        ...
        if (bytes == null) {
            return ResultDTO.error("模型数据为空，请先设计流程并成功保存，再进行发布。");
        }
        ...
        if (model.getProcesses().size() == 0) {
            return ResultDTO.error("数据模型不符要求，请至少设计一条主线流程。");
        }
